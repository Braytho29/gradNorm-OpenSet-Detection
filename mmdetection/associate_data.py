...
def associate_detections(dataHolder, dets, gt, known_count, unknown_count, bbox_known, bbox_unknown, clsCutoff = 15):
		gtBoxes = gt['bboxes']
		gtLabels = gt['labels']
		...
		detConfsGradNorm = dets['confs_gradnorm']
		detimname = dets['imname']
				...
				for iouIdx in sorted_iouIdxs:
					...
					if ious[iouIdx] >= iouThresh:
						...
						if detPredict[detIdx] == gtLbl:
							...
							dataHolder['type'] += [0]
							dataHolder['confs_gradnorm'] += [detConfsGradNorm[detIdx]]
							dataHolder['imname'] += [detimname]

						if detConfsGradNorm[detIdx] > 15000:
							# print(f'Known Class: {detimname}, conf: {detConfsGradNorm[detIdx]}, bbox: {detBoxes}, class: {gtLabels}, index: {detIdx}') 
							known_count += 1
		...
		if np.sum(detAssociated) == len(detAssociated):
			return dataHolder, known_count, unknown_count, bbox_known, bbox_unknown
			...
			for idx, score in enumerate(sorted_scores):
				...
				for iouIdx in sorted_iouIdxs:
					if ious[iouIdx] >= iouThresh:
						...
						dataHolder['confs_gradnorm'] += [detConfsGradNorm[detIdx]]
						dataHolder['imname'] += [detimname]
						print(f'Unknown Class: {detimname}, conf: {detConfsGradNorm}, bbox: {detBoxes}, class: {gtLabels}, index: {detIdx}')
						print(f'conf: {detConfsGradNorm}, logits: {detLogits[detIdx]} index: {detIdx}')
						unknown_count += 1
						# if detConfsGradNorm[detIdx] < 10000:
...
if args.train:
	...
	for tIdx, trainDataset in enumerate(trainDatasets):
		...
		for imIdx, im in enumerate(tqdm.tqdm(trainDataset, total = lenDataset)):
			...
			detBoxes = detData[:, -7:-3]
			detScores = detData[:, -2]
			detConfGradnorm = detData[:,-1]
			detLogits = detData[:, :-7]
			detPredict = np.argmax(detLogits, axis = 1)

			# print(f'boxes: {detBoxes}')
			# print(f'scores: {detScores}')
...
for typIdx, nm in enumerate(['val', 'test']):
	...
	allData = {'scores': [], 'type': [], 'logits': [], 'ious': [], 'confs_gradnorm': [], 'imname': []}
	...
	for imIdx, im in enumerate(tqdm.tqdm(testDataset, total = lenDataset)):
		...
		detConfGradnorm = detConfGradnorm[mask]
		allDetsIm = {'predictions': detPredict, 'scores': detScores, 'boxes': detBoxes, 'logits': detLogits, 'confs_gradnorm': detConfGradnorm, 'imname': imName}
		

		
		#associate detections to objects
		allData_results = associate_detections(allData, allDetsIm, gtData, known_count, unknown_count, bbox_known, bbox_unknown, clsCutoff = num_classes)
		allData = allData_results[0]
		known_count = allData_results[1]
		unknown_count = allData_results[2]
		bbox_known = allData_results[3]
		bbox_unknown = allData_results[4]

	print(f'number: {(bbox_known)} vs below thresh: {known_count}')
	print(f'number: {(bbox_unknown)} vs below thresh: {unknown_count}')
